...
export const defaultNavigationLinks: Navbar03NavItem[] = [
...
export const Navbar03 = React.forwardRef<HTMLElement, Navbar03Props>(
  (
    ...
    // NOTE: explicit `HTMLElement | null` so `.current` is mutable (not read-only)
    ...
    // Combine refs (safe assignment to passed `ref`)
    const combinedRef = React.useCallback(
      (node: HTMLElement | null) => {
        // write to our local mutable ref
        containerRef.current = node;

        // write to the forwarded ref (handle function refs and object refs)
        if (typeof ref === 'function') {
          ref(node);
        } else if (ref) {
          // cast to MutableRefObject so TS lets us assign
      ...
      >
        <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {isMobile && (
              <Popover>
                {/* Use a native <button> here (DOM elements accept refs). 
                    Avoid passing a custom Button component that might not forward refs. */}
                <PopoverTrigger asChild>
                  <button
                    type="button"
                    aria-label="Open menu"
                    className="group h-9 w-9 hover:bg-accent hover:text-accent-foreground inline-flex items-center justify-center rounded-md"
                  >
                    <HamburgerIcon />
                  </button>
                </PopoverTrigger>
